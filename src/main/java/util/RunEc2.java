...
public class RunEc2 {
        ...
        public static void main(String[] args) {
            ...
            try {
                ...
                credentials = new ProfileCredentialsProvider("mah1042").getCredentials();
            } catch (Exception e) {
            ...
            try {
                ...
                RunInstancesResult result = ec2.runInstances(
                        runInstancesRequest);
                System.out.println("EC2 instance successfully running! InstanceId : " + result.getReservation().getInstances().get(0).getInstanceId());
                System.out.println("Retrieving Public IP Address...");
                String publicIPv4 = null;
                int retry = 3;
                int waitTime = 5; //exponential backoff;
                while(publicIPv4 == null && retry>0) {
                    try {
                        TimeUnit.SECONDS.sleep(waitTime);
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                    }
                    DescribeInstancesRequest request = new DescribeInstancesRequest().withInstanceIds(result.getReservation().getInstances().get(0).getInstanceId());
                    DescribeInstancesResult result1 = ec2.describeInstances(request);
                    List<Reservation> list = result1.getReservations();
                    for (Reservation res : list) {
                        List<Instance> instanceList = res.getInstances();
                        for (Instance instance : instanceList) {
                            publicIPv4 = instance.getPublicIpAddress();
